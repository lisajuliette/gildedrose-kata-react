...
describe('Random items', () => {
	test('Reduces quality and days to sell of any item', () => {
...
describe('Conjured items', () => {
	...
	test('Cannot reduce quality to below 0', () => {
		const items = updateItems([
			{ name: CONJURED_PREFIX + 'manacake', sellIn: 0, quality: 1 },
		]);
...
describe('Backstage passes', () => {
	const BACKSTAGE_PASSES = BACKSTAGE_PASSES_PREFIX + 'for TAFKAL80ETC concert';
	...
	test('Resets quality to zero after the concert has passed', () => {
		const items = updateItems([
